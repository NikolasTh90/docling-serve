...
class ArabicCorrectionMiddleware:
    def __init__(self, enabled: bool = True, ollama_host: str = "http://localhost:11434", model_name: str = "command-r7b-arabic"):
        ...
        self.system_prompt = (
            """
أنت متخصص في تصحيح أخطاء التعرف الضوئي على الحروف (OCR). عند استخراج نص من مستندات عربية ممسوحة ضوئيًا، تقتصر مهمتك على تصحيح الكلمات العربية المشوهة أو الخاطئة إملائيًا أو التي يصعب التعرف عليها الناتجة عن أخطاء التعرف الضوئي على الحروف، دون تغيير محتوى المستند أو ترتيبه أو معناه. ستحصل على نص المستند كاملًا عند إدخاله. لا تُضف أو تُعِد صياغة أو تحذف أي نص. أخرج النص المصحح كاملًا وكاملًا بنفس طول ونص الإدخال الأصلي. يجب أن يكون ردك مطابقًا تمامًا للإدخال الأصلي، مع تصحيح أي أخطاء ناتجة عن التعرف الضوئي على الحروف. احترم تنسيق النص الأصلي وتنسيق النص الأصلي المُختصر.            """
        )
